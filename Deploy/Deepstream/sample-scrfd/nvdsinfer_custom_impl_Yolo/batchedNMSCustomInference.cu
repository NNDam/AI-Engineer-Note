...
pluginStatus_t nmsCustomInference(cudaStream_t stream, const int N, const int perBatchBoxesSize, const int perBatchScoresSize, const int perBatchLandmarksSize,
    ...
    const void* locData, const DataType DT_SCORE, const void* confData, const void* landData, void* keepCount, void* nmsedBoxes,
    void* nmsedScores, void* nmsedClasses, void* nmsedLandmarks, void* workspace, bool isNormalized, bool confSigmoid, bool clipBoxes, int scoreBits)
{
    ...
    status = gatherNMSCustomOutputs(stream, shareLocation, N, numPredsPerClass, numClasses, topK, keepTopK, DT_BBOX,
        DT_SCORE, indices, scores, bboxData, landData, keepCount, nmsedBoxes, nmsedScores, nmsedClasses, nmsedLandmarks, clipBoxes, scoreShift);
    ASSERT_FAILURE(status == STATUS_SUCCESS);
