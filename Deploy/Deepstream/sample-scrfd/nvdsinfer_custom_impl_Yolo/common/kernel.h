...
pluginStatus_t nmsCustomInference(cudaStream_t stream, int N, int boxesSize, int scoresSize, int landmarksSize, bool shareLocation,
    int backgroundLabelId, int numPredsPerClass, int numClasses, int topK, int keepTopK, float scoreThreshold,
    float iouThreshold, DataType DT_BBOX, const void* locData, DataType DT_SCORE, const void* confData, const void* landData, void* keepCount,
    void* nmsedBoxes, void* nmsedScores, void* nmsedClasses, void* nmsedLandmarks, void* workspace, bool isNormalized = true,
    bool confSigmoid = false, bool clipBoxes = true, int scoreBits = 16);
...
size_t detectionForwardLandmarkPermuteSize(bool shareLocation, int N, int C3, DataType DT_BBOX);
