...
def allocate_buffers(engine, max_boxes, total_classes):
    ...
    for binding in engine:
        ...
        assert min(binding_shape) > 0, print(binding, binding_shape)
        size = 1
        for i in range(len(binding_shape)):
            size *= binding_shape[i]
...
class TrtModel(object):
    ...
    def run(self, input, deflatten: bool = True, as_dict = False):
        ...
        if deflatten:
            trt_outputs = [output[:np.prod(shape)].reshape(shape) for output, shape in zip(trt_outputs, self.out_shapes)]
        if as_dict:
